    ...
    for(int index = 0; index < doc.size(); index++)
    {
        if(doc[index] == QChar::ObjectReplacementCharacter)
        {
            ...
            while(indexUrl < count)
            {
                ...
                if(this->document()->toHtml().contains(msg.msgcontent, Qt::CaseSensitive))
                {
                    indexUrl++;
...
void MessageTextEdit::insertImages(const QString &url)
{
    ...
    if(image.width() > 120 || image.height() > 80)
    {
        if(image.width() > image.height())
            image = image.scaledToWidth(120, Qt::TransformationMode::SmoothTransformation);
        else
...
{
    MsgInfo msgInfo{.msgFlag = flag, .msgcontent = text, .pixmap = pix};
    list.append(msgInfo);
}
...
QPixmap MessageTextEdit::getFileIconPixmap(const QString &url)
{
    ...
    QString strFileSize = getFilesSize(fileInfo.size());
...
QString MessageTextEdit::getFilesSize(qint64 size)
{
    QString unit;
    double num;
    if(size < 1024)
